...
3. Immediately before each separately authorized live batch, run a one-prompt, no-search synthetic smoke test through the intended live backend in the same execution/network context. For the HUIT proxy, prefer the proven direct-SDK Responses backend; use the wrapper preflight only when the historical GABRIEL backend is explicitly selected. Success requires nonempty response text, a response ID when available, no `Connection error.`, output tokens greater than zero, and `model_response_succeeded` or equivalent success metadata. A prior batch's success does not satisfy this step.
...
5. Release only a small, separately authorized live slice selected from the manifest and current scout-coverage gaps.
6. Quarantine all returned rows as scout-stage output, add them to the durable national queue, apply light metadata/calibration triage, and update municipality/state/county scout coverage. Record parseable empty results and exclude connection-only failures from discovery coverage.
7. Continue national scouting before routine deep verification. Select later coordinated verification waves from high-value matched sets and calibration needs; stop scaling if scout metadata or later sampled verification shows material wrong-employer or wrong-unit leakage.

The Texas, successful Massachusetts rerun, and New Jersey direct-SDK runs now feed the national queue and coverage layer. The project is ready to resume national scaling only in small, separately authorized state slices with a fresh smoke preflight and immediate queue/coverage accounting. This contract does not justify a blind national batch, remove later human verification, or authorize ingestion.
