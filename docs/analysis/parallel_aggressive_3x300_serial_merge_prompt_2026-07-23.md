# Future Coordinator Prompt — Aggressive 3×300 Serial Accounting Merge

Use only after every live lane for
`POST-PI-AGGRESSIVE-ROUND-3X300-2026-07-23` has terminated and a complete
offline lane audit exists. This is the separate serial accounting authorization
boundary; do not run it during preparation or live collection.
...
Read the committed manifest, all three locked inputs/audits, every lane output
and result review, `parallel_lane_audit_summary.json`,
`parallel_lane_audit_report.md`, and `merge_recommendation.md`.

Recompute:

- the three 300-row locked input hashes;
- current input eligibility and cross-lane municipality/Census uniqueness;
...
- each lane-local timestamped candidate export and byte identity with the
  corresponding `parsed_candidates.csv`;
...
explicitly approves the named completed-only subset. If any partial lane has
parseable rows, any artifact is ambiguous, or the recommendation is
`do_not_merge_until_resume_or_review`, do not run shared builders.

## Exactly one serial accounting rebuild

Preserve audited lane candidate exports and state-usage artifacts as
deterministic coordinator inputs. Register only official lane lineages and
terminal failures; never register diagnostic or probe output.
...
Refresh yield learning, dashboard/project-phase JSON, combined runtime/yield
status, and official progress toward 2,000. Decide priority refresh using the
unchanged documented 300–600-success cadence or another explicit deterministic
strategy trigger. Do not change methodology.

## Required post-checkpoint transition

Clearly report whether the approximately 2,000-covered target was exceeded and
by how much. If the merge reaches or exceeds the checkpoint:

- mark the source-discovery scale-up checkpoint reached;
- pause broad ordinary scouting;
- do not prepare another broad discovery round automatically;
- create a post-checkpoint transition note for:
  ...
  documentation → dashboard wage-growth-gap filtering; and
- keep regressions deferred.

Broad scouting may continue only if the user or PI explicitly authorizes a new
discovery strategy after reviewing the post-checkpoint cycle.
...
Create one local merge commit and a complete relay with lane/audit lineage,
