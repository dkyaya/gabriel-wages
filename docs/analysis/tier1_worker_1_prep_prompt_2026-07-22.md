# Tier 1 Worker 1 Offline Preparation Prompt

Use Codex Routine / GPT-5.6 Terra Medium.

Work only in `/Users/joachimjohnson/Documents/RA_2026/Pol_Fire/parallel_worktrees/gabriel-worker-1`. This is offline preparation and prompt dry-run review only. Do not run a smoke, live scout, API/model request, hosted search, URL opening/download, source verification, public-records action, ingestion, `gabriel.codify`, queue/coverage rebuild, dashboard build, or protected canonical edit. Do not inspect/configure/validate/modify remotes; do not push, fetch, or pull.
...
git switch -C tier1_worker_1_prep_20260722 main
test "$(git branch --show-current)" = "tier1_worker_1_prep_20260722"
...
```bash
test -f AGENTS.md
test -f docs/analysis/tier1_worker_1_scout_input_2026-07-22.csv
test -f docs/analysis/tier1_worker_1_input_audit_2026-07-22.md
test -f docs/analysis/tier1_worker_1_prep_prompt_2026-07-22.md
...
- Input: `docs/analysis/tier1_worker_1_scout_input_2026-07-22.csv`
- Expected rows: 50
...
- Expected rank span: `1–50`
- Expected input SHA-256: `2828934d7185a437cbd961d16363812f81889f63a20ff77b4c332da463abf606`
...
path = Path("docs/analysis/tier1_worker_1_scout_input_2026-07-22.csv")
...
assert [int(r["tier1_rank"]) for r in rows] == list(range(1, 51))
...
assert hashlib.sha256(path.read_bytes()).hexdigest() == "2828934d7185a437cbd961d16363812f81889f63a20ff77b4c332da463abf606"
print("PASS: Tier 1 Worker 1 locked-input contract")
PY
```
...
Require `tmp/tier1_worker_1_prep_dry_run_20260722_attempt1` not to exist. Then run exactly:

```bash
test ! -e tmp/tier1_worker_1_prep_dry_run_20260722_attempt1
...
Inspect `tmp/tier1_worker_1_prep_dry_run_20260722_attempt1/prompt_preview.md`, `tmp/tier1_worker_1_prep_dry_run_20260722_attempt1/row_timing.csv`, and `tmp/tier1_worker_1_prep_dry_run_20260722_attempt1/run_metadata.json`. Create `docs/analysis/tier1_worker_1_filter_contract_dry_run_review_20260722_attempt1.md`.
...
Create `docs/analysis/tier1_worker_1_no_network_validation_20260722_attempt1.md` with commands, exit codes, concise output, Python executable, and an explicit statement that no network/API/model/backend call occurred. Do not run a smoke preflight or any live/direct-SDK invocation.
...
```bash
git add docs/analysis/tier1_worker_1_filter_contract_dry_run_review_20260722_attempt1.md docs/analysis/tier1_worker_1_no_network_validation_20260722_attempt1.md
git diff --cached --name-only
git commit -m "Prepare Tier 1 Worker 1 offline dry run"
git status --short
```
...
RELAY="tmp/tier1_worker_1_prep_relay_2026-07-22_${COMMIT}.zip"
STAGE="tmp/tier1_worker_1_prep_relay_2026-07-22_${COMMIT}"
test ! -e "$STAGE"
test ! -e "$RELAY"
mkdir -p "$STAGE/docs/analysis" "$STAGE/dry_run" "$STAGE/evidence"
cp docs/analysis/tier1_worker_1_scout_input_2026-07-22.csv docs/analysis/tier1_worker_1_input_audit_2026-07-22.md docs/analysis/tier1_worker_1_prep_prompt_2026-07-22.md "$STAGE/docs/analysis/"
cp docs/analysis/tier1_worker_1_filter_contract_dry_run_review_20260722_attempt1.md docs/analysis/tier1_worker_1_no_network_validation_20260722_attempt1.md "$STAGE/docs/analysis/"
cp -R tmp/tier1_worker_1_prep_dry_run_20260722_attempt1/. "$STAGE/dry_run/"
...
printf "%s\n" "Worker 1 prep only is complete. Coordinator must inspect this relay and owns any later dry merge, smoke, or live scout." > "$STAGE/next_task.md"
zip -qr "$RELAY" "$STAGE"
...
The final relay basename must remain `tier1_worker_1_prep_relay_2026-07-22_<commit>.zip` in the coordinator repo's `tmp/`. This copy is mandatory. Before copying, inspect ZIP filenames (not credential contents) and stop if `.env`, credential, token, cookie, secret, `.venv`, or cache files appear.
