...
**Last updated:** 2026-07-14 (Worcester/Arlington codify pass) — see the changelog at the bottom for the full revision history and what changed in each pass. `data/contracts.csv` has 64 rows across 19 cities in 5 states (MA, OH, TX codified; PA, NJ source-availability/partially-ingested). **15 cities are codified** (real, verified GABRIEL evidence-layer rows exist): 8 MA (Arlington, Boston, Franklin, Georgetown, Seekonk, Somerville, Wayland, Worcester), 4 OH (Cincinnati, Cleveland, Columbus, Toledo), 3 TX (Austin, Houston, San Antonio) — counted directly from `docs/analysis/gabriel_codify_evidence_layer.csv`'s distinct (state, city) pairs; an earlier version of this file's header stated "16 cities... 9 MA" for the pre-Worcester/Arlington state, which undercounted incorrectly (the true pre-session figure was 13: 6 MA + 4 OH + 3 TX) — corrected here rather than left to compound. Philadelphia PA and 3 NJ cities (Newark, Trenton, and — candidate-only — Jersey City) have real ingested `data/contracts.csv` rows but **zero codified evidence** — every PA/NJ claim below is a source-availability/design-level hypothesis, not coded evidence, and is labeled as such throughout. Do not cite any PA/NJ finding in this file as codified evidence in a future report unless a later changelog entry confirms a codify wave ran.
...
## Worcester, MA — **codified 2026-07-14 (thin, by construction)**

**Sources**
- `ma_worcester_fire_2017` (IAFF Local 1009, 2017-2020), `ma_worcester_clerical_admin_2017` (Local 490, 2017-2020), `ma_worcester_public_works_2017` (Local 170, 2017-2020) — all in `data/contracts.csv`, all now codified (5 live GABRIEL/codify calls total this session across Worcester + Arlington; see `docs/analysis/gabriel_codify_worcester_arlington_audit_2026-07-14.md`).

**Claims / Relevant findings**
Contributes modestly to `CLM-2026-07-12-03` (MA cross-occupation base): 13 present, cleanly-grounded evidence rows across the three occupation classes (fire: overtime/benefits/subcontracting-civilianization/management-rights/civil-service-funding/budget; clerical: a probationary-period grievance-arbitration exclusion, classification, benefits, a bargaining-unit-scope reclassification; public_works: overtime, classification, budget). **Do not treat this as a rich matched-triad result** — see Limits below.

**Limits / Gaps**
**All three Worcester documents are wage/benefits amendment MOAs to a prior base contract not in this corpus, not full base CBAs** — the fire and clerical MOAs are explicitly "off the record... not binding until approved," and the public_works MOA is titled "DRAFT #1" on its own cover. They amend specific articles (wages, health insurance, leave, one new probationary-period article, one bargaining-unit-scope change) rather than restating recognition/no-strike/grievance-procedure/minimum-staffing articles — codify correctly returned `not_found` for those attributes across all three documents, reflecting what these specific documents cover, not an absence in Worcester's underlying labor relationships. No police leg. Worcester should be cited as **breadth-of-cities** context for CLM-03, not as a rich mechanism source the way Arlington now is.

## Arlington, MA — **codified 2026-07-14 — a genuine, rich matched pair**

**Sources**
- `ma_arlington_fire_2021` (IAFF Local 1297, FY2022-2024) and `ma_arlington_public_works_2021` (AFSCME Local 680, 2021-2024) — both codified this session; this is Arlington's one true exact-cycle matched pair (confirmed by `ingest/audit_coverage.py`). `ma_arlington_public_works_2015`/`_2018` (earlier, non-overlapping DPW cycles) remain ingested but deliberately not codified this session (no matched-pair value for the current fire cycle; see the audit doc for the full selection rationale).
- `ma_arlington_public_works_2021`'s source PDF (35 pages) originally extracted to only 5,620 usable characters via the standard pipeline (most pages lack a text layer) — recovered via a bounded, single-pass OCR pass this session (105,305 characters), making this row usable for the first time.

**Claims / Relevant findings**
**The corpus's richest genuine matched PAIR result** (as opposed to a triad or a single strong-but-unmatched source): fire and public_works share 5 present, cleanly-grounded attributes — `civil_service_or_statutory_employment_channel`, `interest_arbitration_or_formal_impasse_backstop`, `management_rights_or_service_flexibility`, `no_strike_or_work_stoppage_constraint`, `overtime_callback_holdover_mandatory_extra_work` — plus fire-only hazard/line-of-duty and union-security evidence and public_works-only classification/premium-pay/subcontracting evidence. All 49 present excerpts across both rows passed grounding cleanly (0 flags). Directly strengthens `CLM-2026-07-12-03`.

**Also a new, non-safety exemplar for `CLM-2026-07-12-06`** (the interest-vs-grievance arbitration distinction): `ma_arlington_public_works_2021` contains, in one document, a Duration-article successor-contract impasse clause ("mediation and fact-finding... Chapter 1078 of the Acts of 1973") coded separately from an Article VII civil-service/disciplinary grievance-arbitration clause — the first time this within-document distinction has been demonstrated in a non-safety document (prior exemplars — San Antonio police, Toledo police, Houston fire, Somerville police — were all safety).

**A genuinely important complication for H1/H2/H7** (not yet reflected in any `CLM-2026-07-12-XX` claim text, since none of those claims are scoped to make this exact comparison): **Arlington's non-safety row shares its safety counterpart's interest-arbitration/impasse-backstop AND no-strike coverage** — both documents reference the same town-wide Massachusetts statutory impasse mechanism (Ch. 1078 of the Acts of 1973, mediation and fact-finding), not a safety-specific channel. This differs from the Ohio triads' asymmetric pattern (`CLM-2026-07-12-01`, safety-only impasse language) underlying H1/H2, and is the **second** non-Ohio matched pair (after Houston TX's non-safety row) found to show this symmetric pattern. See `hypothesis_tracker_2026-07-12.csv` H1/H2/H7/H8 for the full updated reasoning — support levels were deliberately left unchanged (2 cross-state examples is a real pattern worth tracking, not yet "regular" enough to downgrade a hypothesis on its own).

**Limits / Gaps**
Excludes Arlington's two earlier, non-overlapping public_works cycles (2015-2018, 2018-2021) — codifying those would add repeat-cycle depth but was out of this session's bounded scope. `data/contracts.csv`'s `text_quality` label for `ma_arlington_fire_2021` (`ocr_messy`) is stale — it actually extracts cleanly via the text layer; not corrected this session (flagged as a data-quality cleanup item). Separately, this session's codify results surfaced a genuine deterministic-extractor gap: `ma_arlington_fire_2021`'s `no_strike_clause_flag=0` in `data/contracts.csv` is a **false negative** — the document contains "There shail be no strikes during the life of this Agreement," but the `no_strike` regex trigger list only matches singular "no strike," not plural "no strikes" (confirmed by direct regex testing; see `wage_mechanism_evidence_checklist.md` item 19). Likely a generalizable gap, not fixed this session (out of this session's authorized scope, which covered GABRIEL/codify, not extractor-code changes).
...
- **Report-ready-adjacent claims today rest entirely on MA/OH/TX** — 15 codified cities (13 before this session + Worcester + Arlington). `CLM-2026-07-12-01` (Ohio matched triads) and `CLM-2026-07-12-06` (arbitration distinction) are the two strongest, `report_ready=yes`; `CLM-2026-07-12-02` (Texas unevenness) is `report_ready=yes` as a design/source claim specifically, not a wage-gap claim. `CLM-2026-07-12-03` (MA cross-occupation base) is materially strengthened this session by Arlington's rich matched-pair result but stays `report_ready=maybe`, deliberately not inflated. `CLM-2026-07-12-07` (comparator evidence) and `CLM-2026-07-12-08` (Texas non-safety outside Houston) are both explicitly `needs_more_evidence`/`report_ready=no` — do not overstate either in any future report.
- **Arlington MA (fire + public_works, 2026-07-14) is the corpus's richest genuine matched PAIR result and a real complication for the Ohio-triad asymmetric-mechanism pattern**: its non-safety row shares its safety counterpart's interest-arbitration/impasse-backstop and no-strike coverage, via a shared MA statutory mechanism (Ch. 1078 of the Acts of 1973) rather than a safety-specific one. This is the second non-Ohio matched pair (after Houston TX's non-safety row) to show this symmetric pattern — see `hypothesis_tracker_2026-07-12.csv` H1/H2/H7/H8. Worcester's three codified rows, by contrast, are wage/benefits amendment MOAs (not base CBAs) and stay thin, breadth-only support for CLM-03.
- **Philadelphia PA and Trenton NJ remain the two strongest candidates for the next codify wave** — both genuinely matched-design (not just occupation-class-present), found via document-level, primary-source-first sourcing (union locals' own contract archives, direct PERC-index browsing). Deliberately NOT codified this session (out of scope for the bounded Worcester/Arlington wave); a Philadelphia/Trenton wave would additionally serve as a third, cross-framework test of the H1/H2/H7 impasse-symmetry question above, since PA (Act 111) and NJ (the Police and Fire Public Interest Arbitration Reform Act) each have their own distinct statutory impasse structure.
...
- **The Columbus/GABRIEL-evidence-layer contamination question is resolved: no contamination found.** See "What would change this ledger" below.
...
6. A Philadelphia/Trenton codify wave, both to move those cities from design-hypothesis to codified evidence and to serve as a third cross-framework test of the H1/H2/H7 impasse-symmetry question raised by Arlington's 2026-07-14 result.
...
- **Worcester/Arlington codify pass (done 2026-07-14):** 5 live calls, 49/49 present excerpts grounded cleanly, 0 flagged rows — the cleanest codify wave to date. Arlington materially strengthened `CLM-2026-07-12-03` and `CLM-2026-07-12-06` and surfaced new counterevidence relevant to H1/H2/H7/H8 (see above). Also surfaced a likely-generalizable gap in the deterministic `no_strike` extractor (plural "no strikes" phrasing isn't matched by any current trigger — `wage_mechanism_evidence_checklist.md` item 19) — not fixed this session (out of scope; a GABRIEL/codify session, not an extractor-code session).
- Fixing the plural "no strikes" extractor gap (checklist item 19) and re-running the full-corpus regression check would likely surface additional corrected `no_strike_clause_flag` values elsewhere in the corpus — worth prioritizing given how common plural phrasing is likely to be.
...
- **2026-07-14 (Worcester/Arlington codify wave)** — A bounded, 5-call GABRIEL/codify wave against Worcester (fire, clerical_admin, public_works) and Arlington (fire, public_works — its one exact-cycle matched pair). 49/49 present excerpts grounded cleanly, 0 flagged rows (see `gabriel_codify_worcester_arlington_audit_2026-07-14.md`). Updated Worcester and Arlington sections above with real findings. Arlington turned out to be the corpus's richest genuine matched-pair result and surfaced a real complication for the Ohio-triad asymmetric-mechanism pattern (its non-safety row shares its safety counterpart's interest-arbitration/impasse-backstop and no-strike coverage) — reflected in updates to `CLM-2026-07-12-01/-03/-06` and `H1/H2/H7/H8` (`notes`/evidence fields only; no support-level or status field was changed, per this session's decision rule not to inflate claims from newly-organized-but-thin evidence). Also surfaced checklist item 19 (a likely-generalizable plural-phrasing gap in the deterministic `no_strike` extractor), not fixed this session. Philadelphia and Trenton remain the strongest next codify-wave candidates, explicitly not touched this session.
