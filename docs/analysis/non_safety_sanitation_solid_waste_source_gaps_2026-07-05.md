...
**Remaining sub-gap:** whether Seekonk's town-employed DPW workers (if they perform collection at all) are distinguishable from a single town-contracted hauler was not resolved from public pages alone — a job-description-level re-read of the already-collected CBA is the recommended next step, not further web search. **2026-07-06 update — Seekonk CBA sanitation-language scan completed.** `seekonk_public_works_sanitation_language_scan_2026-07-06.md` (with a companion `.csv`) read the full Seekonk public works CBA for explicit and bundled sanitation language. Finding: no explicit sanitation terminology anywhere in the document; two possible-but-unconfirmed signals: (1) "Transfer Station" is explicitly named in the hours-of-work section with different scheduling (any 5 consecutive days excluding Sunday), but no linked job title or duty description; (2) CDL training reimbursement is mentioned for "eligible employees," indicating truck-operation roles exist, but not explicitly linked to sanitation. Overall classification: `sanitation_possible_but_unconfirmed`, pending Appendix verification and Seekonk's own service-structure clarification (municipal-staffed vs. contractor).
