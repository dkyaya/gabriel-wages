...
- This report analyzes **GABRIEL-coded evidence patterns** drawn from 37 codified municipal labor agreements across Massachusetts, Texas, and Ohio — 293 coded "present" findings across 19 wage-mechanism attributes, of which 284 pass an automated source-grounding check and carry no reviewer flag ("verified present").
- It compares **safety** (police, fire), **safety-adjacent** (EMS/nurse-health), and **non-safety** (teachers, public works, library, clerical/admin, and broad "other" municipal units) wage-setting sources on the same coded attributes.
- **The evidence supports a mechanism story, not definitive causal proof.** GABRIEL/codify records whether specific mechanism language appears in a curated source window; it does not measure how much any mechanism actually moved a wage outcome, and it cannot rule out a wage effect where no language was coded.
- Police/fire sources more often show **formal impasse/interest-arbitration backstops**, explicit **coverage/staffing** obligations, **overtime/premium-pay architecture**, and institutional bargaining-power channels than the non-safety sources coded so far — though the margins for several of these attributes are moderate, not stark, and grievance arbitration itself is common across both groups.
- Non-safety sources most often show **classification/wage-grade structure**, **benefits/total compensation**, and **management-rights** language — channels that convert pressure into pay differently, not evidence of an absence of pressure.
- **Ohio now provides the cleanest matched triads** in this corpus: all four codified Ohio cities (Columbus, Cleveland, Cincinnati, Toledo) have a police, a fire, and a non-safety row, coded under an identical statutory framework (ORC Chapter 4117).
- **Texas illustrates institutional unevenness**: only Houston has a genuine non-safety codified source; Austin's comparison runs through EMS (safety-adjacent, not ordinary civilian/clerical); San Antonio contributes police/fire institutional contrast only, deliberately unmatched.
- **Massachusetts provides the densest cross-occupation comparison** — Franklin and Seekonk each span five occupation classes in one city and cycle window, and Somerville's police arbitration award and Wayland's OCR-recovered dispatch/nurse content are the corpus's most mechanism-rich individual documents.
...
GABRIEL's `codify()` function was run against curated, source-grounded text windows (short verbatim excerpts built from `pdftotext`/OCR extractions of each contract, never the full document) against a fixed 19-attribute wage-mechanism codebook — covering peer/comparator wage comparison, interest/impasse arbitration, grievance arbitration, staffing/coverage/overtime, premium pay, classification, training/credential premiums, hazard rationale, benefits, subcontracting, management rights, no-strike clauses, civil-service/statutory channels, union security, budget/fiscal constraints, non-safety administrative wage-restraint channels, and a catch-all "other."

For each attribute, `codify()` returns a binary judgment (`present`/`not_found`, occasionally `unclear`) plus a short verbatim excerpt when present. Every returned excerpt is automatically checked against its source window for an exact substring match (`source_grounding_status=grounded`/`unsupported`); a second layer catches excerpts that echo this project's own codebook vocabulary or leak across window-section boundaries, flagging them for human review rather than silently trusting them (`notes_flag`/`viewer_verified`). This report treats only `viewer_verified=1` present rows as evidence.

The durable evidence layer (`docs/analysis/gabriel_codify_evidence_layer.csv`) unions five separate codify runs (pilot; Texas/Ohio scale-up; Massachusetts; Seekonk/Wayland; expanded Texas/Ohio) into one table, and the PI-facing viewer (`docs/analysis/gabriel_codify_excerpt_browser_latest.html`) lets a reader browse every coded excerpt, filtered by state/city/occupation/mechanism, with unverified rows hidden by default.

**Limitations of the method itself:** codify is a binary presence detector operating over hand-curated windows, not the full document — a `not_found` result can mean the mechanism is genuinely absent, or that it simply wasn't included in this session's curated excerpt selection. Text quality varies (a few sources are OCR-recovered and imperfect). None of this supports a causal wage estimate on its own.
...
Massachusetts is this corpus's **densest cross-occupation laboratory**: Franklin (police, fire, public works, library, other) and Seekonk (police, fire, teacher, public works, library) each span five occupation classes within a single city and overlapping cycle window, giving the report its most direct same-city, same-period safety-vs-non-safety comparisons. **Somerville's police JLMC arbitration award** is the corpus's clearest interest-arbitration document — a full award opinion, not just a contract clause, explicitly reasoning through the "potential threat to public welfare" standard. **Wayland's fire JLMC stipulated award and its OCR-recovered dispatch/nurse content** (`ma_wayland_other_2021`, recovered via a bounded OCR pass after the original PDF extracted to ~0 usable characters) together make Wayland this corpus's most mechanism-rich single city for testing the safety/civilian-adjacent boundary. Massachusetts's non-safety sources (Boston clerical/admin, Franklin library/public_works/other, Seekonk teacher/library/public_works, Georgetown other) most often show classification and administrative-channel language rather than arbitration language — consistent with this project's standing finding (independently verified across four non-safety groups in earlier sessions) that none of Massachusetts's non-safety occupation groups has JLMC-equivalent compulsory interest arbitration.

![Massachusetts cross-occupation matrix](report_assets/massachusetts_cross_occupation_matrix_2026-07-10.png)

### Texas

**Houston is Texas's only fully matched city** in this codified set — police, fire (an arbitration award, not a base CBA), and a genuine non-safety row (`tx_houston_other_2024`, HOPE/AFSCME Local 123). **Austin's comparison runs through EMS**, not an ordinary civilian/clerical unit: Austin EMS is civil-service-protected and statutorily adjacent to police/fire (shared Chapter 143 Civil Service Commission), a caveat that must travel with any Austin-EMS finding rather than being generalized to "Texas non-safety." **San Antonio contributes police/fire institutional contrast only** — it was deliberately added to test whether Houston's population-triggered compulsory-arbitration exception (Sec. 174.1535, applying only to cities ≥1.9M) generalizes, not as a third matched Texas triad; no non-safety bargaining channel was confirmed to exist for San Antonio despite a live search. Texas's overall mechanism profile in this corpus should be read with real caution given how few non-safety/adjacent sources exist: the "Texas non-safety" and "Texas safety-adjacent" bars in several of this report's charts represent a single source each (Houston HOPE; Austin EMS), so any 0%/100% reading is a small-sample artifact, not a robust state-level finding.
