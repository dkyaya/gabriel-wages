        ...
        <main>
            ...
            <div>
              ...
              <strong>
                {projectPhaseSummary.whole_corpus_claim_package_prep_available
                  ...
                  : projectPhaseSummary.whole_corpus_synthesis_available
                  ? `${formatNumber(projectPhaseSummary.whole_corpus_rated_span_count)} canonical rated spans · ${formatNumber(projectPhaseSummary.whole_corpus_source_count)} batch-local sources · mechanism gate ${projectPhaseSummary.whole_corpus_gate_statuses?.mechanism_evidence_gate ?? "unavailable"}`
                  ...
                  : projectPhaseSummary.remaining_municipality_quantitative_normalization_matching_available
                  ? `${formatNumber(projectPhaseSummary.remaining_municipality_normalized_quantitative_record_count)} normalized · ${formatNumber(projectPhaseSummary.remaining_municipality_provisional_local_comparison_count)} provisional + ${formatNumber(projectPhaseSummary.remaining_municipality_conditional_local_comparison_count)} conditional local comparisons · ${formatNumber(projectPhaseSummary.remaining_municipality_quant_qual_link_count)} quant–qual assessments`
                  : projectPhaseSummary.remaining_municipality_normalization_matching_prep_available
                  ? `${formatNumber(projectPhaseSummary.remaining_municipality_expanded_structural_comparison_count)} structural candidates from ${formatNumber(projectPhaseSummary.remaining_municipality_explicit_comparison_seed_count)} explicit seeds · ${formatNumber(projectPhaseSummary.remaining_municipality_normalization_prep_universe_count)} normalization prep · ${formatNumber(projectPhaseSummary.remaining_municipality_matching_prep_universe_count)} matching prep`
                  : projectPhaseSummary.remaining_municipality_side_reconciliation_available
                  ? `${formatNumber(projectPhaseSummary.remaining_municipality_side_reconciliation_inspected_count)} unclear spans inspected · 0 excluded upfront · ${formatNumber(projectPhaseSummary.remaining_municipality_clear_side_quantitative_candidate_count)} clear-side quantitative candidates`
                  : projectPhaseSummary.remaining_municipality_rating_ingestion_available
                  ? `${formatNumber(projectPhaseSummary.remaining_municipality_ingested_source_count)} ingested sources · ${formatNumber(projectPhaseSummary.remaining_municipality_ingested_span_count)} valid ratings · ${formatNumber(projectPhaseSummary.remaining_municipality_unclear_side_reconciliation_queue_count)} unclear queued in full`
                  : projectPhaseSummary.remaining_municipality_gabriel_rating_available
                  ? `${formatNumber(projectPhaseSummary.remaining_municipality_gabriel_rated_source_count)} rated sources · ${formatNumber(projectPhaseSummary.remaining_municipality_gabriel_rated_span_count)} valid ratings · ${formatNumber(projectPhaseSummary.remaining_municipality_gabriel_quarantine_span_count)} quarantined`
          ...
          <details className="pi-technical-details">
            ...
            <div className="pi-technical-grid">
              ...
              {projectPhaseSummary.remaining_municipality_normalization_matching_prep_available && <>
                <div><span>Explicit comparison seeds</span><strong>{formatNumber(projectPhaseSummary.remaining_municipality_explicit_comparison_seed_count)}</strong></div>
                <div><span>Expanded structural prep</span><strong>{formatNumber(projectPhaseSummary.remaining_municipality_expanded_structural_comparison_count)}</strong></div>
                <div><span>Normalization-prep universe</span><strong>{formatNumber(projectPhaseSummary.remaining_municipality_normalization_prep_universe_count)}</strong></div>
                <div><span>Matching-prep universe</span><strong>{formatNumber(projectPhaseSummary.remaining_municipality_matching_prep_universe_count)}</strong></div>
              </>}
              {projectPhaseSummary.remaining_municipality_quantitative_normalization_matching_available && <>
                <div><span>Normalization input</span><strong>{formatNumber(projectPhaseSummary.remaining_municipality_normalization_input_count)}</strong></div>
                <div><span>Normalized records</span><strong>{formatNumber(projectPhaseSummary.remaining_municipality_normalized_quantitative_record_count)}</strong></div>
                <div><span>All-tier structures processed</span><strong>{formatNumber(projectPhaseSummary.remaining_municipality_all_tier_matching_input_count)}</strong></div>
                <div><span>Local comparison candidates</span><strong>{formatNumber(projectPhaseSummary.remaining_municipality_provisional_local_comparison_count + projectPhaseSummary.remaining_municipality_conditional_local_comparison_count)}</strong></div>
                <div><span>Quant–qual link assessments</span><strong>{formatNumber(projectPhaseSummary.remaining_municipality_quant_qual_link_count)}</strong></div>
                <div><span>National-readiness candidates</span><strong>{formatNumber(projectPhaseSummary.remaining_municipality_national_readiness_candidate_count)}</strong></div>
              </>}
              {projectPhaseSummary.remaining_municipality_blocker_rescue_reclassification_available && <>
                <div><span>Blocker signatures</span><strong>{formatNumber(projectPhaseSummary.remaining_municipality_unique_blocker_signature_count)}</strong></div>
                <div><span>Rescued / rerouted structures</span><strong>{formatNumber(projectPhaseSummary.remaining_municipality_records_structures_rescued_rerouted)}</strong></div>
                <div><span>Cleaned analysis-use units</span><strong>{formatNumber(projectPhaseSummary.remaining_municipality_cleaned_analysis_use_count)}</strong></div>
                <div><span>Direct / conditional routes</span><strong>{formatNumber(projectPhaseSummary.remaining_municipality_cleaned_direct_comparison_count)} / {formatNumber(projectPhaseSummary.remaining_municipality_cleaned_conditional_comparison_count)}</strong></div>
                <div><span>Same-side scalar / structured</span><strong>{formatNumber(projectPhaseSummary.remaining_municipality_cleaned_same_side_scalar_count)} / {formatNumber(projectPhaseSummary.remaining_municipality_cleaned_structured_schedule_count)}</strong></div>
                <div><span>Growth / quant–qual routes</span><strong>{formatNumber(projectPhaseSummary.remaining_municipality_cleaned_growth_count)} / {formatNumber(projectPhaseSummary.remaining_municipality_cleaned_quant_qual_count)}</strong></div>
              </>}
              {projectPhaseSummary.remaining_municipality_local_comparison_qa_available && <>
                <div><span>Local QA pool</span><strong>{formatNumber(projectPhaseSummary.remaining_municipality_local_comparison_qa_input_count)}</strong></div>
                <div><span>Local claim / supporting / conditional</span><strong>{formatNumber(projectPhaseSummary.remaining_municipality_local_claim_ready_count)} / {formatNumber(projectPhaseSummary.remaining_municipality_local_supporting_example_count)} / {formatNumber(projectPhaseSummary.remaining_municipality_conditional_example_count)}</strong></div>
              ...
              {projectPhaseSummary.whole_corpus_synthesis_available && <>
                <div><span>Canonical layers / rated spans</span><strong>{formatNumber(projectPhaseSummary.whole_corpus_canonical_layer_count)} / {formatNumber(projectPhaseSummary.whole_corpus_rated_span_count)}</strong></div>
                <div><span>Sources / claim-readiness units</span><strong>{formatNumber(projectPhaseSummary.whole_corpus_source_count)} / {formatNumber(projectPhaseSummary.whole_corpus_claim_readiness_record_count)}</strong></div>
                <div><span>Claim / supporting / conditional</span><strong>{formatNumber(projectPhaseSummary.whole_corpus_claim_boundary_counts?.claim_ready)} / {formatNumber(projectPhaseSummary.whole_corpus_claim_boundary_counts?.supporting_example_ready)} / {formatNumber(projectPhaseSummary.whole_corpus_claim_boundary_counts?.conditional_with_caveats)}</strong></div>
                <div><span>Mechanism / synthesis gates</span><strong>{projectPhaseSummary.whole_corpus_gate_statuses?.mechanism_evidence_gate ?? "unavailable"} / {projectPhaseSummary.whole_corpus_gate_statuses?.whole_corpus_synthesis_gate ?? "unavailable"}</strong></div>
                <div><span>Local / national readiness</span><strong>{projectPhaseSummary.whole_corpus_gate_statuses?.local_comparison_gate ?? "unavailable"} / {projectPhaseSummary.whole_corpus_gate_statuses?.national_readiness_gate ?? "unavailable"}</strong></div>
                <div><span>Global wage-gap / causal</span><strong>{projectPhaseSummary.whole_corpus_gate_statuses?.global_wage_gap_readiness_gate ?? "unavailable"} / {projectPhaseSummary.whole_corpus_gate_statuses?.global_causal_readiness_gate ?? "unavailable"}</strong></div>
              </>}
              {projectPhaseSummary.whole_corpus_claim_package_prep_available && <>
                <div><span>Supportable / conditional mechanisms</span><strong>{formatNumber(projectPhaseSummary.major_supportable_causal_mechanism_claim_count)} / {formatNumber(projectPhaseSummary.major_conditional_causal_mechanism_claim_count)}</strong></div>
