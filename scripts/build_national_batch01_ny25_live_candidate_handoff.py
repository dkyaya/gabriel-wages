#!/usr/bin/env python3
"""Build the normalized, unverified NY25 scout-candidate handoff CSV.
...
RUN_DIR = (
    ...
    / "NY"
    / "national_batch01_ny25_live_direct_sdk_2026-07-20"
)
...
OUTPUT = (
    ...
    / "docs"
    / "analysis"
    / "national_batch01_ny25_live_direct_sdk_scout_candidates_2026-07-20.csv"
)
EXPECTED_ROWS = 57
...
def verification_notes(row: dict[str, str]) -> str:
    ...
    if row.get("unit_type") == "unclear":
        prefix += "Resolve the unit label before matched-set use. "
    return (
        prefix
...
def build_rows() -> list[dict[str, str]]:
    ...
    if len(source_rows) != EXPECTED_ROWS:
        raise ValueError(
            f"Expected {EXPECTED_ROWS} live NY candidates, found {len(source_rows)}"
        )

    output: list[dict[str, str]] = []
    for row in source_rows:
        if row.get("state") != "NY":
            raise ValueError(f"Unexpected state in NY handoff input: {row.get('state')}")
        if not row.get("source_url", "").strip():
            raise ValueError(
                f"Candidate lacks source URL: {row.get('municipality')} / "
...
def main() -> int:
    ...
    if len(parsed) != EXPECTED_ROWS:
        raise ValueError("NY handoff row count changed on parse-back")
    if set(parsed[0]) != set(FIELDS):
        raise ValueError("NY handoff schema changed on parse-back")
    if {row["scout_stage_status"] for row in parsed} != {
        "unverified_scout_candidate"
    }:
        raise ValueError("NY handoff contains a non-scout-stage status")
