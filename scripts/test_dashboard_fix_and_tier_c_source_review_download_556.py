#!/usr/bin/env python3
"""Regression tests for bounded source review/download over 556 verified leads."""
...
RUNNER = ROOT / "scripts/run_dashboard_fix_and_tier_c_source_review_download_556.py"
SPEC = importlib.util.spec_from_file_location("dashboard_fix_tier_c_download_556", RUNNER)
assert SPEC and SPEC.loader
mod = importlib.util.module_from_spec(SPEC)
sys.modules[SPEC.name] = mod
SPEC.loader.exec_module(mod)
DASHBOARD_RUNNER = ROOT / "scripts/build_dashboard_data.py"
DASHBOARD_SPEC = importlib.util.spec_from_file_location("dashboard_fix_tier_c_download_556_dashboard", DASHBOARD_RUNNER)
...
class DashboardFixTierCSourceReviewDownload556Tests(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.queue, cls.hashes = mod.verify_inputs()

    def test_task_lineage_and_scope(self) -> None:
        self.assertEqual(mod.INPUT_COMMIT, "24615facf4d2efd18e9976d0ae0033946cf71715")
        self.assertEqual(len(self.queue), 556)
    ...
    def test_only_verified_tier_c_enters(self) -> None:
        ...
        self.assertEqual({mechanism: sum(row["target_mechanism_family"] == mechanism for row in self.queue) for mechanism in mod.EXPECTED_MECHANISMS}, mod.EXPECTED_MECHANISMS)
        self.assertEqual({region: sum(row["derived_region"] == region for row in self.queue) for region in mod.EXPECTED_REGIONS}, mod.EXPECTED_REGIONS)

    def test_nonverified_results_are_excluded(self) -> None:
        all_rows = mod.read_csv(mod.INPUT_DIR / "targeted_tier_c_verification_results.csv")
        ...
        self.assertEqual(len(excluded), 444)
    ...
    def test_result_row_keeps_all_downstream_boundaries_closed(self) -> None:
        ...
        result = mod.result_row(row, raw, "2026-07-27T00:00:00Z")
    ...
    def test_duplicate_hash_quarantine_removes_redundant_copy(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temp:
            ...
            base = {"source_review_download_status": "retained_downloaded_source", "file_sha256": mod.sha256(first),
                    "priority_tier": "tier_c", "duplicate_file_group_id": "", "notes": "", "candidate_id": "A"}
            rows = [{**base, "local_retained_path": "retained/a.pdf"},
                    {**base, "candidate_id": "B", "local_retained_path": "retained/b.pdf"}]
    ...
    def test_prepared_lock_when_present(self) -> None:
        path = mod.OUTPUT_DIR / "targeted_tier_c_source_review_download_556_lock.json"
        if not path.exists():
            self.skipTest("dry preparation has not run")
        lock = mod.read_json(path)
        locked = mod.read_csv(mod.OUTPUT_DIR / "targeted_tier_c_source_review_download_556_locked_queue.csv")
        self.assertEqual(len(locked), 556)
    ...
    def test_completed_outputs_when_present(self) -> None:
        decision_path = mod.OUTPUT_DIR / "dashboard_fix_and_tier_c_source_review_download_556_decision.json"
        if not decision_path.exists():
            self.skipTest("live download has not completed")
        mod.validate_complete()
        decision = mod.read_json(decision_path)
        self.assertEqual(decision["locked_download_queue_count"], 556)
    ...
    def test_dashboard_visibility_fix_is_current(self) -> None:
        phase = mod.read_json(ROOT / "docs/dashboard/data/project_phase_summary.json")
        self.assertTrue(mod.dashboard_visibility_ready() or phase.get("current_phase_code") == "dashboard_declutter_map_correction_tier_c_text_span_completed_rating_ready")
        self.assertEqual(phase["data_vintage"], "2026-07-27")
        self.assertIn(phase["current_phase_code"], {
        ...
        self.assertTrue(phase["pdf_text_layer_readiness_ready_next"])
        self.assertFalse(phase["global_analysis_readiness"])

    def test_current_deploy_bundle_has_no_stale_header(self) -> None:
        ...
        self.assertIn("Tier C text and exact-span extraction complete", bundle)
    ...
    def test_remote_diagnostics_record_read_only_settings(self) -> None:
        ...
        self.assertEqual(diagnostics["pages_build_type"], "workflow")
        self.assertEqual(diagnostics["workflow_artifact_path"], "docs/dashboard/dist")
        self.assertFalse(diagnostics["remote_settings_changed"])
        self.assertFalse(diagnostics["git_fetch_or_pull_used"])
...
if __name__ == "__main__":
    suite = unittest.defaultTestLoader.loadTestsFromTestCase(DashboardFixTierCSourceReviewDownload556Tests)
