#!/usr/bin/env python3
"""Build the normalized, unverified CA25 scout-candidate handoff CSV.

This is a deterministic local-only transform. It does not open URLs, verify
sources, ingest documents, edit canonical data, submit records requests, or
codify text.
"""
...
RUN_DIR = (
    ...
    / "CA"
    / "national_batch01_ca25_live_direct_sdk_2026-07-20"
)
...
OUTPUT = (
    ...
    / "docs"
    / "analysis"
    / "national_batch01_ca25_live_direct_sdk_scout_candidates_2026-07-20.csv"
)
EXPECTED_ROWS = 64
...
def verification_notes(row: dict[str, str]) -> str:
    ...
    return (
        prefix
        + "Later coordinated verification must confirm the exact California municipal "
        "employer and bargaining unit, authoritative provenance, executed/binding and "
        "complete-document status, visible operative dates, wage-setting content, "
        "duplicate status, and same-city cycle overlap. Do not ingest or use for claims "
        "before that gate; do not initiate a public-records request."
    )
...
def build_rows() -> list[dict[str, str]]:
    ...
    if len(source_rows) != EXPECTED_ROWS:
        raise ValueError(
            f"Expected {EXPECTED_ROWS} live CA25 candidates, found {len(source_rows)}"
        )

    output: list[dict[str, str]] = []
    for row in source_rows:
        if row.get("state") != "CA":
            raise ValueError(f"Unexpected state in CA25 input: {row.get('state')}")
...
def main() -> int:
    ...
    if len(parsed) != EXPECTED_ROWS:
        raise ValueError("CA25 handoff row count changed on parse-back")
    if list(parsed[0]) != FIELDS:
        raise ValueError("CA25 handoff schema changed on parse-back")
    if {row["scout_stage_status"] for row in parsed} != {
        "unverified_scout_candidate"
    }:
        raise ValueError("CA25 handoff contains a non-scout-stage status")
    if any(not row["source_url"].strip() for row in parsed):
        raise ValueError("CA25 handoff unexpectedly contains a missing source URL")
    print(f"candidate_rows={len(parsed)}")
    print("queueable_source_rows=64 missing_locator_rows=0")
