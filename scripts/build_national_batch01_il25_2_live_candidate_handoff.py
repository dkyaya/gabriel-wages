#!/usr/bin/env python3
"""Build the normalized, unverified IL25.2 scout-candidate handoff CSV.
...
RUN_DIR = (
    ...
    / "IL"
    / "national_batch01_il25_2_live_direct_sdk_2026-07-20"
)
...
OUTPUT = (
    ...
    / "docs"
    / "analysis"
    / "national_batch01_il25_2_live_direct_sdk_scout_candidates_2026-07-20.csv"
)
EXPECTED_ROWS = 72
...
def build_rows() -> list[dict[str, str]]:
    ...
    if len(source_rows) != EXPECTED_ROWS:
        raise ValueError(
            f"Expected {EXPECTED_ROWS} live IL25.2 candidates, found {len(source_rows)}"
        )

    output: list[dict[str, str]] = []
    for row in source_rows:
        if row.get("state") != "IL":
            raise ValueError(f"Unexpected state in IL25.2 input: {row.get('state')}")
...
def main() -> int:
    ...
    if len(parsed) != EXPECTED_ROWS:
        raise ValueError("IL25.2 handoff row count changed on parse-back")
    if set(parsed[0]) != set(FIELDS):
        raise ValueError("IL25.2 handoff schema changed on parse-back")
    if {row["scout_stage_status"] for row in parsed} != {
        "unverified_scout_candidate"
    }:
        raise ValueError("IL25.2 handoff contains a non-scout-stage status")
