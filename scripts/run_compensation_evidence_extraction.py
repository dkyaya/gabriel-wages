...
def prompt(row: dict[str, str], pages: list[PagePacket]) -> str:
    ...
    return (
        ...
        "Benefits, overtime, stipends, longevity, certification, pension, leave, reimbursements, healthcare, and uniforms belong only in non_base_wage_observations unless base-pay evidence is separately present. "
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    p.add_argument("--mode", choices=("freeze_500_selection", "preflight", "live_lanes"), required=True)
...
def main(argv: list[str] | None = None) -> int:
    args = parse_args(argv); output = args.output_dir.resolve(); gate3 = args.gate3_ledger.resolve()
    if args.case_limit != 500: raise ValueError("this task requires exactly 500 cases")
    ...
    selection = load_selection(output); packet_rows, packet_map = packet_rows_and_map(output, selection)
